...
class Settings(BaseSettings):
    ...
    # --- System Prompts ---
    SYSTEM_PROMPT_PLANNER: str = (
        "You are a creative world-building assistant. Your goal is to outline consistent and interesting wiki articles."
    )
    SYSTEM_PROMPT_WRITER: str = (
        "You are an encyclopedic writer. Write detailed, dry, but descriptive wiki articles based on the provided outline."
    )
    ...
    def configure_provider_settings(self):
        ...
        match self.AI_PROVIDER:
            ...
            case "xai":
                ...
                if not self.LLM_MODEL:
                    self.LLM_MODEL = "grok-4-1-fast-reasoning-latest"
                if not self.IMAGE_GEN_MODEL:
